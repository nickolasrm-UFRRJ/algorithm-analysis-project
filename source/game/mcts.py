...
class MCTSTree(BaseTree, NodeMixin):  # Add Node feature
...
def MCTS(tree):
    ...
    if node.is_leaf: #the node is a leaf: we need to find out its situation(bad or good)
        result = node.simulate(node) #A.I. will simulate and give a score to the node
    ...
    node.children = node.expand(node) #expands the current node
    result = node.select_node() #selects the best node to be simulated
    result = node.simulate(result)
